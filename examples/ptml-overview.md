---
theme: pitch-deck-vc
title: PTML — The Post-PPTX Presentation System
author: Pete Shimshock
themeList: minimal-white,pitch-deck-vc,dracula,tokyo-night,aurora,corporate-clean,swiss-grid
---

<!-- layout: cover -->
<!-- notes: Welcome to PTML. Today we're showing why HTML presentations are the future. -->

# PTML

HTML-based presentations for the post-PPTX era

---

<!-- layout: content -->

[Why HTML]
## Why HTML Presentations?

Business users are ready to do away with PPTX files. Here is why HTML is the answer.

- **Cross-platform compatibility** — Runs natively in all modern browsers without plugins
- **Device flexibility** — Works on desktops, laptops, tablets, and smartphones
- **Easy sharing** — Simple HTTP links replace complex file attachments
- **Live editing** — Write in Markdown, see instant preview
- **Reduced security risks** — No macro-enabled files or executable content
- **Performance** — Fast load times and smooth transitions
- **Shareable links** — Generate a short link anyone can view in their browser

---

<!-- layout: content -->

[Roadmap]
## Project Stages

^Agenda

1. Develop or adopt code framework
2. Create agentic tooling (skills)
3. Build presentation builder
4. Build presentation viewer
5. Free for everyone — all features, no tiers

---

<!-- layout: big-quote -->
<!-- notes: Close with a strong statement about the future. -->

> Ready to do away with PPTX files?
> Let's build the future of presentations.
