...
(function (global) {
  ...
  function buildHTML(parsed, options) {
    ...
    return [
      ...
      '  <style>',
      '    /* FOUC prevention — hide deck until CSS + runtime are ready */',
      '    .deck{visibility:hidden}',
      '    .ptml-loader{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:#0a0a0c;z-index:9999;transition:opacity .4s ease}',
      '    .ptml-loader.hidden{opacity:0;pointer-events:none}',
      '    .ptml-spinner{width:28px;height:28px;border:3px solid rgba(255,255,255,0.1);border-top-color:#3b6cff;border-radius:50%;animation:ptml-spin .7s linear infinite}',
      '    @keyframes ptml-spin{to{transform:rotate(360deg)}}',
      '  </style>',
      ...
      '<div class="ptml-loader" id="ptml-loader"><div class="ptml-spinner"></div></div>',
      '<div class="deck">',
      '',
                 parsed.slidesHTML,
      '',
      ...
      '<script>',
      '  // Remove loader once runtime is ready',
      '  window.addEventListener("load", function() {',
      '    var deck = document.querySelector(".deck");',
      '    var loader = document.getElementById("ptml-loader");',
